...
>>>>>>> 7dab38d073c4f2d25cdb2f8f59254fc632cc28e2
...
<<<<<<< HEAD
const getTypeStyle = (t) => EVENT_TYPES.find(x => x.value === t)?.color || 'bg-gray-100 text-gray-700 border-gray-200'
const getTypeLabel = (t) => EVENT_TYPES.find(x => x.value === t)?.label || t
=======
...
export default function CalendarView({ projectId, project }) {
  ...
  const addEvent = async (e) => {
    ...
    await addDoc(collection(db, 'projects', projectId, 'events'), {
      title: form.title.trim(), type: form.type, description: form.description.trim(),
      date: form.date, createdAt: serverTimestamp(),
    })
    setShowForm(false)
  }
...
>>>>>>> 7dab38d073c4f2d25cdb2f8f59254fc632cc28e2
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        ...
        <div className="grid grid-cols-7 bg-gray-50">
          {WEEKDAYS.map((w, i) => (
            <div key={w} className={`text-center text-xs font-medium py-2 ${i === 0 ? 'text-red-500' : i === 6 ? 'text-blue-500' : 'text-gray-500'}`}>{w}</div>
          ))}
        </div>
        <div className="grid grid-cols-7">
          {Array.from({ length: firstDayOfWeek }).map((_, i) => (
<<<<<<< HEAD
            <div key={`e-${i}`} className="min-h-[80px] border-r border-b border-gray-100 bg-gray-50" />
=======
...
>>>>>>> 7dab38d073c4f2d25cdb2f8f59254fc632cc28e2
          ...
          {Array.from({ length: daysInMonth }, (_, i) => i + 1).map(day => {
            ...
            return (
<<<<<<< HEAD
              <div key={day} className="min-h-[80px] border-r border-b border-gray-100 p-1 hover:bg-blue-50 cursor-pointer transition" onClick={() => openForm(day)}>
                <div className={`text-xs font-medium w-6 h-6 flex items-center justify-center rounded-full mb-1 ${isToday ? 'bg-blue-600 text-white' : dow === 0 ? 'text-red-500' : dow === 6 ? 'text-blue-500' : 'text-gray-700'}`}>{day}</div>
                <div className="space-y-0.5">
                  {dayEvents.slice(0, 3).map(ev => (
                    <div key={ev.id} onClick={e => { e.stopPropagation(); deleteEvent(ev.id) }}
                      ...
                      title={`${ev.title}（クリックで削除）`}>{ev.title}</div>
=======
              <div
                key={day}
...
>>>>>>> 7dab38d073c4f2d25cdb2f8f59254fc632cc28e2
                  ...
                  {dayEvents.length > 3 && <div className="text-xs text-gray-400">+{dayEvents.length - 3}件</div>}
                </div>
              </div>
            )
          })}
...
<<<<<<< HEAD
            {events.filter(e => e.date.startsWith(`${year}-${String(month + 1).padStart(2, '0')}`)).map(ev => (
              <li key={ev.id} className="flex items-start gap-3">
                <span className={`text-xs px-2 py-0.5 rounded border flex-shrink-0 mt-0.5 ${getTypeStyle(ev.type)}`}>{getTypeLabel(ev.type)}</span>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <form onSubmit={addEvent} className="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl">
            ...
            <input type="date" className="w-full border rounded-lg px-3 py-2 mb-3 focus:outline-none focus:ring-2 focus:ring-blue-400" value={form.date} onChange={e => setForm(f => ({ ...f, date: e.target.value }))} />
            <input className="w-full border rounded-lg px-3 py-2 mb-3 focus:outline-none focus:ring-2 focus:ring-blue-400" placeholder="タイトル" value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} autoFocus />
            <select className="w-full border rounded-lg px-3 py-2 mb-3 focus:outline-none focus:ring-2 focus:ring-blue-400" value={form.type} onChange={e => setForm(f => ({ ...f, type: e.target.value }))}>
              {EVENT_TYPES.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}
            </select>
            <input className="w-full border rounded-lg px-3 py-2 mb-4 focus:outline-none focus:ring-2 focus:ring-blue-400" placeholder="メモ (任意)" value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} />
