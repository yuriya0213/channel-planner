...
export default function TodoView({ projectId }) {
  ...
  const addTodo = async (e) => {
    ...
    setForm({ title: '', memberId: '', dueDate: '' })
    setShowForm(false)
  }

  const toggleDone = async (id, done) => {
    await updateDoc(doc(db, 'projects', projectId, 'todos', id), { done: !done })
  }

  const deleteTodo = async (id) => {
    await deleteDoc(doc(db, 'projects', projectId, 'todos', id))
  }
    ...
    <div>
      ...
      <div className="flex gap-2 mb-4 flex-wrap">
        <button
          ...
          className={`px-3 py-1.5 rounded-full text-sm font-medium border transition ${filterMember === 'all' ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-600 border-gray-300 hover:bg-gray-50'}`}
        >
          全員
        </button>
        <button
          onClick={() => setFilterMember('unassigned')}
          className={`px-3 py-1.5 rounded-full text-sm font-medium border transition ${filterMember === 'unassigned' ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-600 border-gray-300 hover:bg-gray-50'}`}
        >
          未割当
        </button>
        {members.map(m => (
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-sm font-medium border transition ${filterMember === m.id ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-600 border-gray-300 hover:bg-gray-50'}`}
          >
            👤 {m.name}
          </button>
        ))}
        <button
          ...
          className="ml-auto px-3 py-1.5 bg-blue-600 text-white rounded-full text-sm font-medium hover:bg-blue-700"
        >
          ＋ タスク追加
        </button>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 mb-3 overflow-hidden">
        ...
        {pending.length === 0 ? (
          <p className="text-sm text-gray-400 text-center py-8">タスクなし 🎉</p>
        ) : (
          <ul className="divide-y divide-gray-50">
            {pending.map(todo => (
              <TodoItem
                key={todo.id}
                todo={todo}
                memberName={getMemberName(todo.memberId)}
...
function TodoItem({ todo, memberName, onToggle, onDelete }) {
  ...
  return (
    <li className="flex items-center gap-3 px-4 py-3 hover:bg-gray-50">
      <button
        onClick={onToggle}
        className={`w-5 h-5 rounded border-2 flex-shrink-0 flex items-center justify-center transition ${todo.done ? 'bg-green-500 border-green-500 text-white' : 'border-gray-300 hover:border-blue-400'}`}
      >
        {todo.done && '✓'}
      </button>
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-0.5">
          {todo.memberId && (
            <span className="text-xs text-blue-600 bg-blue-50 px-1.5 py-0.5 rounded">👤 {memberName}</span>
          )}
          {todo.dueDate && (
            <span className={`text-xs px-1.5 py-0.5 rounded ${isOverdue ? 'text-red-600 bg-red-50' : 'text-gray-400 bg-gray-100'}`}>
